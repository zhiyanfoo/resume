...
\section{Experience}
  ...
  \resumeSubheading
    ...
    {Software Infrastructure Engineering Intern}{Sept 2019 - Dec 2019 and May 2020 - August 2020}
        \begin{itemize}[label={\textbullet}]
          \item Reworked deploy process to use a distributed build process (via Concourse). 
            Led to 50\% reduction in deploy times for microservices (20m to 10m) and enabled multiple parallel builds.
          \item Built a new service (Go) that allowed modifications of our customer's Kubernetes ingress,
            so that they could filter traffic to their sites.
          ...
          \item Increased on-call alerting coverage via Prometheus and Elasticsearch.
            Standardized builds on Buildkit from standard Docker and Rocker.
  ...
  %       \begin{itemize}[label={\textbullet}]
  %         \item Implemented auto deployment on our EU cluster and built tooling for both expiration
  %           and hard-deletion of customer sites (Go, Ruby, AWS CloudFormation).
  %       \end{itemize}
    \resumeSubheading
      {Setter}{Toronto, ON}
